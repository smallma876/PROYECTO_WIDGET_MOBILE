...
describe('DeleteStickerUseCase', () => {
  let mockStickerRepo: jest.Mocked<StickerRepository>;
  let useCase: DeleteStickerUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new DeleteStickerUseCase(mockStickerRepo);
  });

  it('should delete an existing sticker', async () => {
    await useCase.execute('sticker-1');

    expect(mockStickerRepo.getById).toHaveBeenCalledWith('sticker-1');
    expect(mockStickerRepo.delete).toHaveBeenCalledWith('sticker-1');
  });
  ...
  it('should throw if sticker ID is empty', async () => {
    ...
    expect(mockStickerRepo.delete).not.toHaveBeenCalled();
  });
  ...
  it('should propagate repository errors', async () => {
    mockStickerRepo.delete.mockRejectedValue(new Error('Delete failed'));

    await expect(useCase.execute('sticker-1')).rejects.toThrow('Delete failed');
  });
});
